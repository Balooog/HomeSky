"""Diagnostic helper for Weather Lake."""
...
def main() -> None:
    ...
    console.print("[bold cyan]Weather Lake Configuration[/bold cyan]")
    console.print_json(json.dumps(config))

    storage = config.get("storage", {})
    db = DatabaseManager(Path(storage.get("sqlite_path", "./data/weather.sqlite")), Path(storage.get("parquet_path", "./data/lake.parquet")))
